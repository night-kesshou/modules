  ...
  request(options, (e,r,d)=>{
    if(e||!d)
      ...
      for(var i=11;i<temp.length-2;i+=8){ // 排除非必要資訊, 因此 i從11開始至 html.group(td).length-2 結束
        grade[0].push({
          type:temp.eq(i+1).text(),
          credit:parseInt(temp.eq(i+2).text()),
          score:parseInt(temp.eq(i+3).text()),
        });
        grade[1].push({
          type:temp.eq(i+4).text(),
          credit:parseInt(temp.eq(i+5).text()),
          score:parseInt(temp.eq(i+6).text()),
        });
        grade['subject'].push(temp.eq(i).text());
        grade['total'].push(temp.eq(i+7).text());
      }
      return callback({
        data:data,
        grade:grade
